...
<section aria-label="Featured" class="container-page pt-6">
    <div class="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        ...
        <!-- Side Promo Banners (Always 2 Columns side-by-side on mobile, stacked on desktop) -->
        <div class="promo-banner-grid {{ $promoBanners->count() === 1 ? '!grid-cols-1' : '' }} h-full">
            @foreach($promoBanners as $promo)
                <a href="{{ $promo->link ?? '/shop' }}" class="promo-banner-item group block">
                    <img src="{{ $promo->image_path }}" alt="Promo Banner" loading="eager" decoding="async" class="transition-transform duration-500 group-hover:scale-105" />
                </a>
            @endforeach
        </div>
    </div>
...
{{-- Page H1 (SEO phase 3). It sits outside every admin toggle around it, so it always renders. --}}
<section class="container-page py-8 text-center">
    <h1 class="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">{{ \App\Support\Seo::HOME_HEADLINE }}</h1>
    <p class="mx-auto mt-2 max-w-3xl text-sm text-muted-foreground">Brand new intact box, without box and certified pre-owned laptops, MacBooks and gadgets from a genuine wholesaler and retailer since 2012.</p>
</section>
