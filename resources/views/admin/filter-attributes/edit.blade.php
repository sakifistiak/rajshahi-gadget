<x-app-layout>
    <div class="p-6 w-full space-y-6">
        <div class="flex items-center justify-between bg-white p-5 rounded-sm border border-gray-200 shadow-sm">
            <div>
                <h1 class="text-xl font-bold text-gray-900">Edit Filter Attribute</h1>
                <p class="text-xs text-gray-500 mt-1">{{ $filterAttribute->label }}</p>
            </div>
        ...
        <div class="bg-white rounded-sm border border-gray-200 shadow-sm p-6 max-w-2xl" x-data="{ type: '{{ old('type', $filterAttribute->type) }}' }">
            <form action="{{ route('admin.filter-attributes.update', $filterAttribute) }}" method="POST" class="space-y-6">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <select name="category_id" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-sm text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $filterAttribute->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="label" value="{{ old('label', $filterAttribute->label) }}" required
                    ...
                    <div>
                        ...
                        <select name="type" x-model="type" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-sm text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                            <option value="range" {{ old('type', $filterAttribute->type) == 'range' ? 'selected' : '' }}>Range (min–max number, e.g. RAM, Storage, DPI)</option>
                            <option value="select" {{ old('type', $filterAttribute->type) == 'select' ? 'selected' : '' }}>Select (fixed list, e.g. Connection: Wired/Wireless)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="match_labels" value="{{ old('match_labels', $filterAttribute->match_labels) }}" placeholder="e.g. RAM, Memory"
                           class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-sm text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                ...
                <div x-show="type === 'range'">
                    ...
                    <input type="text" name="unit" value="{{ old('unit', $filterAttribute->unit) }}" placeholder="e.g. GB"
                ...
                <div x-show="type === 'select'" style="display:none">
                    ...
                    <input type="text" name="options" value="{{ old('options', $filterAttribute->options) }}" placeholder="e.g. Wired, Wireless, Bluetooth"
                           class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-sm text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" min="0" value="{{ old('sort_order', $filterAttribute->sort_order) }}"
                           class="w-full max-w-xs px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-sm text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                </div>
                ...
                <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-sm shadow-sm transition-all cursor-pointer">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
