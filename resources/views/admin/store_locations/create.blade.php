    ...
    <div class="w-full space-y-6">
        ...
        <form action="{{ route('admin.store-locations.store') }}" method="POST" class="bg-white p-6 rounded-sm border border-gray-100 shadow-sm space-y-5">
            ...
            <div class="space-y-1.5">
                ...
                <div id="quill-editor-address" class="bg-white rounded-sm text-xs border border-gray-200" style="min-height: 120px;">{!! old('address') !!}</div>
                <textarea name="address" id="address-hidden" class="hidden">{{ old('address') }}</textarea>
            </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    var quill = new Quill('#quill-editor-address', {
                        theme: 'snow',
                        placeholder: 'Enter physical address (e.g. House 12, Road 5, Dhanmondi, Dhaka)...',
                        modules: {
                            toolbar: [
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center gap-4 text-xs font-semibold text-gray-700">
                        ...
                        <label class="flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="phone_link_type" value="tel" {{ old('phone_link_type', 'tel') === 'tel' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                            Phone dialer
                        </label>
                        <label class="flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="phone_link_type" value="whatsapp" {{ old('phone_link_type', 'tel') === 'whatsapp' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                            WhatsApp
                        </label>
                    </div>
                </div>
