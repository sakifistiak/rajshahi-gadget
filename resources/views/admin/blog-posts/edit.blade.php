<x-app-layout>
    <div class="w-full space-y-6" x-data="{ imagePath: '{{ old('featured_image_path', $post->featured_image) }}' }">
        <div class="flex items-center justify-between bg-white p-5 rounded-sm border border-slate-200 shadow-sm">
            <div>
                <h1 class="text-xl font-bold text-slate-900">Edit Blog Post</h1>
                <p class="text-xs text-slate-500 mt-1">{{ $post->title }}</p>
            </div>
        ...
        <form action="{{ route('admin.blog-posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-sm border border-slate-200 shadow-sm space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Excerpt <span class="text-rose-500">*</span></label>
                <textarea id="excerpt" name="excerpt" rows="2" required maxlength="500"
                          class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">{{ old('excerpt', $post->excerpt) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input id="category_tag" type="text" name="category_tag" value="{{ old('category_tag', $post->category_tag) }}" required
                           class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Read Minutes</label>
                    <input id="read_minutes" type="number" name="read_minutes" min="1" max="60" value="{{ old('read_minutes', $post->read_minutes) }}"
                           class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="author_name" type="text" name="author_name" value="{{ old('author_name', $post->author_name) }}" required
                           class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Author Role</label>
                    <input id="author_role" type="text" name="author_role" value="{{ old('author_role', $post->author_role) }}"
                           class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="p-4 bg-slate-50 rounded-sm border border-slate-200">
                ...
                <p class="text-[11px] text-slate-500 mb-3">Upload a new file, paste a path/URL, or leave as-is to keep the current image.</p>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 space-y-2">
                        ...
                        <input type="text" placeholder="or paste an image path/URL" x-model="imagePath"
