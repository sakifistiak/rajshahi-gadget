<x-app-layout>
    <div class="w-full space-y-6" x-data="{ imagePath: '{{ old('image_path', $feedback->image) }}' }">
        <div class="flex items-center justify-between bg-white p-5 rounded-sm border border-slate-200 shadow-sm">
            <h1 class="text-xl font-bold text-slate-900">Edit Customer Feedback</h1>
        ...
        <form action="{{ route('admin.customer-feedbacks.update', $feedback) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-sm border border-slate-200 shadow-sm space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-sm shadow-sm transition-all flex items-center gap-2">
                    <i data-lucide="check" class="w-4 h-4"></i>{{ __('Update Feedback') }}
                </button>
            </div>
        </form>
    </div>
