    ...
    <div class="w-full space-y-6" x-data="{ imagePath: '' }">
        ...
        <form action="{{ route('admin.customer-feedbacks.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-sm border border-slate-200 shadow-sm space-y-6">
            ...
            <div>
                ...
                <textarea name="message" rows="3" maxlength="2000" autofocus placeholder="What did the customer say or title?" class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">{{ old('message') }}</textarea>
            </div>
