<x-app-layout>
    <div class="p-6 w-full space-y-6" x-data="{ 
        ...
        imagePath: '{{ old('image_path', $promo->image_path) }}',
        mediaFiles: [],
        loadingMedia: false,
        fetchMedia() {
        ...
        <div class="flex items-center justify-between bg-white p-5 rounded-sm border border-gray-200 shadow-sm">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Update side promo banner image, click URL or active status</p>
            </div>
        ...
        <div class="bg-white rounded-sm border border-gray-200 shadow-sm p-6">
            <form action="{{ route('admin.promos.update', $promo) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="link" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">2. Click Target Link (Where image redirects)</label>
                        <input type="text" name="link" id="link" value="{{ old('link', $promo->link) }}"
                               placeholder="e.g. /shop or /shop?condition=intact"
                        ...
                        <p class="text-[11px] text-gray-400 mt-1">Users will be navigated to this link when clicking the promo banner</p>
                        @error('link') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                ...
                <div class="pt-4 border-t border-gray-100 flex items-center justify-between">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-xs font-bold text-gray-800 uppercase tracking-wider">Active Banner</span>
                    </label>
