<x-app-layout>
    <div class="p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
            <div>
                <h1 class="text-xl font-bold text-gray-900">Edit Side Promo Banner</h1>
                <p class="text-xs text-gray-500 mt-1">Update promo card title, offer, image and link</p>
            </div>
            ...
            <form action="{{ route('admin.promos.update', $promo) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" required value="{{ old('title', $promo->title) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $promo->subtitle) }}"
                               class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="image_path" id="image_path" required value="{{ old('image_path', $promo->image_path) }}"
                    ...
                    <div>
                        ...
                        <select name="bg_color" id="bg_color" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                            <option value="from-sky-100 to-sky-50" {{ $promo->bg_color === 'from-sky-100 to-sky-50' ? 'selected' : '' }}>Soft Sky Blue</option>
                            <option value="from-orange-200 to-orange-100" {{ $promo->bg_color === 'from-orange-200 to-orange-100' ? 'selected' : '' }}>Warm Orange</option>
                            <option value="from-emerald-100 to-emerald-50" {{ $promo->bg_color === 'from-emerald-100 to-emerald-50' ? 'selected' : '' }}>Mint Emerald</option>
                            <option value="from-purple-100 to-purple-50" {{ $promo->bg_color === 'from-purple-100 to-purple-50' ? 'selected' : '' }}>Royal Purple</option>
                            <option value="from-rose-100 to-rose-50" {{ $promo->bg_color === 'from-rose-100 to-rose-50' ? 'selected' : '' }}>Rose Red</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="link" id="link" value="{{ old('link', $promo->link) }}"
                               class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" min="0" value="{{ old('sort_order', $promo->sort_order) }}"
                               class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                    </div>
                    ...
                    <div class="flex items-center pt-6">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $promo->is_active ? 'checked' : '' }}>
