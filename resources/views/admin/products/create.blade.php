<x-app-layout>
    <div class="w-full space-y-6" x-data="{
        ...
        <div class="flex items-center justify-between bg-white p-5 rounded-sm border border-slate-200 shadow-sm">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Create a new product listing for the storefront catalog</p>
            </div>
        ...
        <!-- Form Card -->
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-sm border border-slate-200 shadow-sm space-y-6">
            @csrf
            ...
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus placeholder="e.g. Dell XPS 14 9440"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="warranty" type="text" name="warranty" value="{{ old('warranty') }}" placeholder="e.g. 1 year service warranty"
                           class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="p-4 bg-slate-50 rounded-sm border border-slate-200">
                ...
                <p class="text-[11px] text-slate-500 mb-3">Upload a new image or choose one already uploaded from the Media Library. Leave empty to use the default placeholder.</p>
            ...
            <div>
                ...
                <select name="button_type" class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                    <option value="auto" {{ old('button_type', 'auto') == 'auto' ? 'selected' : '' }}>Auto — follow the Pre-Order checkbox above</option>
                    <option value="buy_now" {{ old('button_type') == 'buy_now' ? 'selected' : '' }}>Always show "Buy Now"</option>
                    <option value="preorder" {{ old('button_type') == 'preorder' ? 'selected' : '' }}>Always show "Pre-Order"</option>
                </select>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            oldGalleryPaths.forEach(path => addGalleryRow(path));

            const oldHighlights = @json(old('highlights', []));
            if (oldHighlights.length > 0) {
                oldHighlights.forEach(val => addHighlightRow(val));
            } else {
                addHighlightRow('');
                addHighlightRow('');
                addHighlightRow('');
            ...
            if (oldColorsHex.length > 0) {
                oldColorsHex.forEach((hex, i) => addColorRow(hex, oldColorsName[i] || ''));
            }
