        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-sm border border-slate-200 shadow-sm space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" required
                          class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all h-32">{{ old('description', $product->description) }}</textarea>
            </div>

            <!-- In Stock Checkbox -->
            <div class="flex items-center gap-2">
                <input id="in_stock" type="checkbox" name="in_stock" value="1" {{ old('in_stock', $product->in_stock) ? 'checked' : '' }}
