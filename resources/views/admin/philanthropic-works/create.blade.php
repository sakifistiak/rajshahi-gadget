<x-app-layout>
    <div class="w-full space-y-6" x-data="{ imagePath: '' }">
        <div class="flex items-center justify-between bg-white p-5 rounded-sm border border-slate-200 shadow-sm">
            <h1 class="text-xl font-bold text-slate-900">Add Philanthropic Work</h1>
        ...
        <form action="{{ route('admin.philanthropic-works.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-sm border border-slate-200 shadow-sm space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required autofocus class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
            </div>
            ...
            <div>
                ...
                <input type="text" name="slug" value="{{ old('slug') }}" class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="place" value="{{ old('place') }}" required placeholder="e.g. Sunamganj" class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                </div>
                <div>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Summary <span class="text-rose-500">*</span> <span class="font-normal normal-case text-slate-400">(short line shown on the listing card)</span></label>
                <textarea name="summary" rows="2" required maxlength="500" class="w-full text-xs px-3.5 py-2.5 rounded-sm border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">{{ old('summary') }}</textarea>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-sm shadow-sm transition-all flex items-center gap-2">
                    <i data-lucide="check" class="w-4 h-4"></i>{{ __('Add Entry') }}
                </button>
            </div>
        </form>
    </div>
