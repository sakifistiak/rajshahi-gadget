<x-app-layout>
    ...
    <div class="bg-white rounded-md border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="px-5 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
                <h3 class="text-[13px] font-bold text-slate-800">{{ __('Philanthropic Work') }}</h3>
                <p class="text-[10px] text-slate-400 mt-0.5">Community initiatives shown on the public /philanthropic-work page, each with its own detail page.</p>
            </div>
            <a href="{{ route('admin.philanthropic-works.create') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold uppercase rounded shadow-sm transition-colors">
                        ...
                        <tr class="hover:bg-slate-50/30 transition-colors">
                            ...
                            <td class="px-6 py-3.5 whitespace-nowrap text-right text-xs font-medium">
                                <div class="flex justify-end items-center gap-3">
                                    ...
                                    <form action="{{ route('admin.philanthropic-works.destroy', $work) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this entry?');" class="inline">
                                        @csrf
                                        @method('DELETE')
