     ...
     <nav class="flex-1 px-3 pb-6 overflow-y-auto space-y-1 text-[13.5px]">
         @if(Auth::user()->is_live_chat_agent && !Auth::user()->is_admin)
             <a href="{{ route('admin.live-chat.index') }}" class="flex items-center justify-between gap-2.5 rounded-md bg-blue-50 px-3 py-2.5 font-bold text-blue-600">
                 ...
                 <span class="live-chat-unread-badge hidden rounded-full bg-rose-600 px-2 py-0.5 text-[10px] font-bold text-white"></span>
             </a>
         @else
         ...
         <div class="pt-1">
             ...
             <div x-show="openSection === 'ecommerce'" x-collapse class="pl-8 pr-2 py-1 space-y-1">
                 ...
                 <a href="{{ route('admin.cart-abandonment.index') }}" class="flex items-center gap-2.5 px-3 py-1.5 rounded-md text-[13px] font-bold transition-colors {{ request()->routeIs('admin.cart-abandonment.*') ? 'bg-blue-50 text-blue-600' : 'text-slate-700 hover:text-blue-600 hover:bg-slate-50' }}">
                     <i data-lucide="alert-triangle" class="h-3.5 w-3.5 {{ request()->routeIs('admin.cart-abandonment.*') ? 'text-blue-600' : 'text-slate-400' }}"></i>
        ...
        <div class="pt-1">
            <button @click="openSection = (openSection === 'live_chat' ? '' : 'live_chat')"
                ...
                <span class="flex items-center gap-1.5">
                    <span class="live-chat-unread-badge hidden rounded-full bg-rose-600 px-1.5 py-0.5 text-[10px] font-bold text-white"></span>
