...
<aside class="hidden lg:flex lg:w-64 lg:flex-col shrink-0 bg-[#ffffff] border-r border-[#e2e8f0]">
    ...
    <nav class="flex-1 px-2.5 pb-4 overflow-y-auto space-y-1">
        ...
        <!-- Homepage Sliders & Banners (Unified Menu with Tabs) -->
        <a href="{{ route('admin.sliders.index') }}" class="flex items-center gap-2.5 px-3 py-2 rounded text-[13px] font-medium transition-colors {{ (request()->routeIs('admin.sliders.*') || request()->routeIs('admin.promos.*')) ? 'bg-[#f1f5f9] text-[#1e293b] font-semibold' : 'text-slate-600 hover:bg-[#f8fafc] hover:text-[#1e293b]' }}">
            <svg class="h-4 w-4 shrink-0 {{ (request()->routeIs('admin.sliders.*') || request()->routeIs('admin.promos.*')) ? 'text-slate-800' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
